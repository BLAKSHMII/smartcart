...
def admin_orders():
    ...
    cursor.execute("""
        SELECT o.order_id, o.amount, o.payment_status, o.created_at,
...
def admin_reset_password():
    ...
    if otp != session.get('admin_reset_otp'):
    ...
    cursor.execute(
        ...
        (hashed_password, session['admin_reset_email'])
    )
    conn.commit()
    cursor.close()
    conn.close()
...
# =================================================================
# ROUTE: SHOW UPDATE FORM WITH EXISTING DATA
# =================================================================
...
#Add a route to let users view past orders:
...
def admin_view_order(order_id):
    ...
    return render_template("admin/view_order.html", order=order, items=items)
